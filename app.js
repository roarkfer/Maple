(() => {
  ...
  let store=loadStore(); let tab='tasks', editing=false, openFolder=null, openHabit=null, draft='';
  let dark=localStorage.getItem(DARK_KEY)==='1'; document.documentElement.classList.toggle('dark',dark);
  ...
  function tasksView(){return `<ul class="list">${store.tasks.map(i=>`<li class="row task-row"><button class="check ${i.done?'on':''}" data-task-toggle="${i.id}">${i.done?'✓':''}</button><button class="task-text ${i.done?'done':''}" data-task-toggle="${i.id}">${esc(i.text)}</button>${editing?`<button class="delete" data-task-del="${i.id}">⌫</button>`:''}</li>`).join('')||'<li class="empty">Lista vacía.</li>'}</ul>`}
  function habitsView(){const t=today(),m=mondayOf(t),tk=dateKey(t),days=Array.from({length:7},(_,i)=>addDays(m,i));return `<ul class="list">${store.habits.map(h=>`<li class="row"><div class="habit-head">${editing?`<input class="habit-name" data-habit-name="${h.id}" value="${esc(h.name)}">`:`<button class="habit-name" data-habit-open="${h.id}">${esc(h.name)}</button>`}${editing?`<button class="delete" data-habit-del="${h.id}">⌫</button>`:''}</div><div class="week-grid">${days.map((d,i)=>{const k=dateKey(d),isToday=k===tk,on=!!h.marks[k];return `<div class="day ${isToday?'today':''}"><span class="day-label">${DAY_LABELS[i]}</span><button class="check ${on?'on':''}" ${isToday?`data-habit-toggle="${h.id}" data-date="${k}"`:'disabled'}>${on?'✓':''}</button></div>`}).join('')}</div></li>`).join('')||'<li class="empty">Sin hábitos.</li>'}</ul>`}
  function foldersView(){return `<ul class="list">${store.folders.map(f=>`<li class="row folder-row"><span class="folder-icon">□</span>${editing?`<input class="folder-name" data-folder-name="${f.id}" value="${esc(f.name)}">`:`<button class="folder-name" data-folder-open="${f.id}">${esc(f.name)}</button>`}<span class="count">${f.exercises.length}</span>${editing?`<button class="delete" data-folder-del="${f.id}">⌫</button>`:''}</li>`).join('')||'<li class="empty">Sin carpetas. Agrega una abajo.</li>'}</ul>`}
  ...
  function exerciseView(folder){return `<div class="list"><div class="exercise-header"><span style="text-align:center">Sets</span><span>Ejercicio</span><span style="text-align:center">Reps</span><span style="text-align:center">Kg</span></div>${folder.exercises.map(ex=>`<div class="row"> <div class="exercise-grid">${stepper(ex.sets,'sets',folder.id,ex.id)}<input class="exercise-name" data-ex-name="${ex.id}" data-folder="${folder.id}" value="${esc(ex.name)}">${stepper(ex.reps,'reps',folder.id,ex.id)}${stepper(ex.kg,'kg',folder.id,ex.id)}</div>${editing?`<button class="delete" data-ex-del="${ex.id}" data-folder="${folder.id}">⌫ Eliminar</button>`:''}</div>`).join('')||'<div class="empty">Sin ejercicios.</div>'}</div>`}
  ...
  function wire(){
    document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{tab=b.dataset.tab;openFolder=null;openHabit=null;editing=false;render()});
